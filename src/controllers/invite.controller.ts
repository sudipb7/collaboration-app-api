...
class InviteController {
  ...
  public getInvites: RequestHandler = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: 'Invites fetched successfully', invites, count, isNext });
    } catch (error) {
  ...
  public createInviteLink: RequestHandler = async (req, res) => {
    try {
      ...
      const isAdmin = workspace.members.some((member) => member.userId === adminId && member.role === 'ADMIN');
      if (!isAdmin) {
