...
class WorkspaceController {
  ...
  public getWorkspaces: RequestHandler = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: 'Workspaces fetched successfully', workspaces, count, isNext });
    } catch (error) {
  ...
  public createWorkspace: RequestHandler = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: 'Workspace created successfully', workspace: updatedWorkspace });
    } catch (error) {
      console.log(JSON.stringify(error));
      return res.status(500).json({ message: 'Internal server error' });
    }
  ...
  public updateWorkspace: RequestHandler = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: 'Workspace updated successfully', workspace: updatedWorkspace });
    } catch (error) {
      console.log(JSON.stringify(error));
      return res.status(500).json({ message: 'Internal server error' });
    }
  ...
  public deleteWorkspace: RequestHandler = async (req, res) => {
    try {
      ...
      const admin = workspace?.members.find((member) => member.userId === adminId && member.role === MemberRole.ADMIN);
      if (!admin) {
